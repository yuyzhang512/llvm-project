...
// Wrap V in the pin carrier and redirect V's other uses to the result.
// Patterns exist for i32-based widths 1/2/4/8/16 lanes only, so a wider value is
// bitcast to <N x i32> and split into those (12 -> 8 + 4).
static bool pinValue(Instruction &I, StringRef MDName, Intrinsic::ID IID) {
  ...
  Value *Vec = B.CreateBitCast(&I, VecTy);

  auto pin = [&](Value *Chunk, unsigned RegNo) {
    Function *Fn = Intrinsic::getOrInsertDeclaration(I.getModule(), IID,
                                                     {Chunk->getType()});
  ...
  for (unsigned Off = 0; Off < Lanes;) {
    ...
    if (W == 1) {
      ...
      Vec = B.CreateInsertElement(Vec, pin(B.CreateExtractElement(Vec, Idx),
                                           Reg + Off),
                                  Idx);
    } else {
      Value *Sub =
          B.CreateExtractVector(FixedVectorType::get(I32, W), Vec, B.getInt64(Off));
      Vec = B.CreateInsertVector(VecTy, Vec, pin(Sub, Reg + Off),
                                 B.getInt64(Off));
    }
    Off += W;
  }
  Value *Pinned = B.CreateBitCast(Vec, Ty);
  for (Use *U : OldUses)
    U->set(Pinned);
  return true;
}
