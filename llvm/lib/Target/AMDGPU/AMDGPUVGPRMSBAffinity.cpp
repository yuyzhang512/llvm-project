...
/// S_SET_VGPR_MSB, which AMDGPULowerVGPREncoding emits whenever a slot's MSB group
/// changes between consecutive instructions.
///
/// This pre-RA pass (run after the scheduler fixes the order) records a desired MSB
/// group per virtual register; SIRegisterInfo's allocation-hint hook then biases
/// the greedy allocator toward it. The hint is soft, so it can never make
/// allocation fail. Steps: build a schedule-driven affinity graph (edges between
/// vregs that would cause a mode switch), cluster it under a per-group
...
class AffinityGraph {
public:
  // Canonical (order-independent) key for the edge between vreg indices A and B.
...
// Union-find over vreg indices that greedily merges the heaviest affinity edges,
// refusing a merge whose footprint would exceed a cap (one MSB group's pressure
// limit). Footprints come from an injected functor so it stays decoupled from
// LiveIntervals.
class ClusterForest {
public:
  ...
  // Greedily merge edges heaviest-first (ties broken by smallest footprint delta,
  // then earliest ordinal, then key for determinism), refusing a merge that would
  // push the merged footprint past MergeCap. Each refused edge is a cut.
  void clusterByWeight(const AffinityGraph &Graph, unsigned MergeCap) {
    ...
    // (Re-)evaluate the merge for an edge against the current forest and enqueue
    // it. Endpoints already in the same cluster are dropped.
...
private:
  // Cluster packing strategy: pack hottest-first into the lowest group that fits
  // (Compact) or into the least-loaded group so clusters spread and every used
  // group keeps slack for the soft hints (Balanced).
  ...
  // number of 256-groups so every cap is 256; at a fractional occupancy the last
  // group holds only VGPRBudget - (EffMSBGroups-1)*256 registers. Clamp to
  ...
  static unsigned groupCap(unsigned Group, unsigned VGPRBudget) {
    int Cap = static_cast<int>(VGPRBudget) -
              static_cast<int>(Group * MSBGroupSize);
    return static_cast<unsigned>(
        std::max(1, std::min<int>(MSBGroupSize, Cap)));
  }
  ...
  // Per-block edge weight: a loop-depth proxy for trip count, so an innermost-loop
  // transition outweighs straight-line code by orders of magnitude.
  ...
  // Value-group union-find (mutable for path compression): vregs that coalesce to
  // one physreg must be counted once in the footprint. See buildValueGroups.
  ...
  unsigned dwords(Register Reg) const {
    // Integer-divide by 32 (a 16-bit vreg -> 0): rounding up over-counts lo16/hi16
    // pairs that share a dword and worsens True16 plans. The footprint only feeds
    // soft hints, so the undercount is acceptable.
    return TRI->getRegSizeInBits(*MRI->getRegClass(Reg)) / 32;
  }

  // Record the MSB-group affinity and also a concrete physreg hint in that group:
  // the latter marks a known preference so greedy colors the vreg early and it
  // claims its group before contention. Existing (copy) hints win.
  ...
  void buildValueGroups(MachineFunction &MF) {
    ...
    for (MachineBasicBlock &MBB : MF) {
      for (MachineInstr &MI : MBB) {
        ...
        // this chains acc0->acc1->... into one value group so the footprint counts
        // the accumulator once. Disjoint output tiles never merge.
  ...
  // Peak simultaneously-live VGPR dwords in \p Regs, merging the live ranges of a
  // value group so a coalescing value is counted once.
  ...
  // Natural (no-hint) MSB-group assignment for the self-benefit baseline: a linear
  // scan placing each vreg in the lowest free column run and freeing columns as
  // live ranges end -- an approximation of what the allocator does unhinted.
  DenseMap<unsigned, int> computeNaiveMSB(ArrayRef<Register> Regs,
    ...
    for (Register R : Order) {
      ...
      int Group;
      if (Start < 0) {
        // No contiguous run fits: reserve Width columns in the least-occupied MSB
        // group so this vreg's footprint stays visible (otherwise the baseline
        // looks artificially uncongested and skews the self-benefit comparison).
  ...
  // simulated like AMDGPULowerVGPREncoding: walk the stream with sticky per-slot
  // state (reset per block), charging blockFreq per instruction that changes a
  // slot's group.
...
bool AMDGPUVGPRMSBAffinity::run(MachineFunction &MF, LiveIntervals *LISIn,
  ...
  // Coalesce vregs that will share one physreg so the footprint counts them once.
  buildValueGroups(MF);

  // If the whole function fits one 256-VGPR group, no S_SET_VGPR_MSB is ever needed
  // and partitioning would only inflate the VGPR count.
  ...
  // Baseline occupancy: min of the VGPR-limited estimate and MFI's non-VGPR limit.
...
AffinityGraph AMDGPUVGPRMSBAffinity::buildAffinityGraph(
  ...
  for (MachineBasicBlock *MBBp : Blocks) {
    ...
    for (MachineInstr &MI : MBB) {
      ...
      // agrees. Weight Freq*144 matches a single-slot boundary so the copy is not
      // under-weighted.
      if (MI.isCopy()) {
      ...
      // Per-slot edges, not deduplicated: a tied accumulator drives both src2 and
      // dst and so contributes 2*Freq to its pair, intentionally emphasizing the
      // accumulator/dst chain over single-slot src0/src1 edges.
      ...
      for (unsigned Slot = 0; Slot < 4; ++Slot) {
        ...
        if (isVGPRVirtReg(R)) {
          ...
          // A physical VGPR pins the slot to a fixed MSB group; break the run so
          // we do not attract vregs across it.
          LastInSlot[Slot] = Register();
        }
        // SGPR / immediate operands leave the slot sticky.
      }
      // Charge the boundary once (one s_set_vgpr_msb covers all changed slots) and
      // distribute its cost across the k changed slots as weight/k, so co-locating
      // a slot that is the sole changer (k=1) is valued fully while batched
      // multi-slot boundaries are discounted. Base 144 = 12^2 keeps the per-slot
      // weight integral for k in 1..4.
      if (!Changed.empty()) {
        ...
        // land in the same group post-RA. Counting dst would misclassify these as
        // multi-slot and skip the boost, leaving the ds_read address stranded in
        // another group (the src0 g0<->g1 flip). Count only src0/src1/src2.
...
AMDGPUVGPRMSBAffinity::collectHotRoots(ClusterForest &Forest,
  ...
  // non-fall-through block entries (including the loop header every iteration), so
  // a value in group 0 needs no switch right after a reset. Keeping the hottest
  // clusters in group 0 therefore minimizes switches.
...
// peak simultaneously-live footprint of its clusters, not the sum of their peaks,
// so time-disjoint clusters (e.g. successive prefetch tiles) share a group instead
// of each reserving 256 registers -- this is what keeps the VGPR count from
// ballooning on software-pipelined kernels.
void AMDGPUVGPRMSBAffinity::packClusters(ClusterForest &Forest,
                                         ArrayRef<unsigned> Roots,
                                         unsigned EffMSBGroups,
                                         unsigned VGPRBudget, PackMode Mode,
                                         MutableArrayRef<int> MSBLoad,
  ...
  for (unsigned Root : Roots) {
    ...
    // group 0, which is reset-free after the loop header). Balanced: place in the
    // group that minimizes the resulting load, so clusters spread across all
    // groups and every used group keeps slack for the soft hints.
...
void AMDGPUVGPRMSBAffinity::processRegion(ArrayRef<MachineBasicBlock *> Blocks,
  ...
  // allocator spills a few, most hints honored); past cap*OverflowPct/100 the plan
  // is unrealizable and the allocator drops the hints. Per-group cap so a
  ...
  // *raise* the switch count, so commit only if the plan's predicted switches beat
  // the natural no-hint layout on this same schedule. Both sims share the naive
  // groups for unhinted vregs, isolating the effect of our hints.
  ...
  if (!Infeasible && BenefitPct) {
    ...
    // Commit only if the plan predicts a large enough win over the naive layout on
    // this exact schedule. Compute the products in 128 bits so the comparison
    // can't overflow on a huge function where the freq-weighted sums are large.
