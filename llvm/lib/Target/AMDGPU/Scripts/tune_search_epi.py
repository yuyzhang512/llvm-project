...
ASM_LAST_WMMA_RE = re.compile(r"^;+\s*WMMA Last Cycle:\s*(\d+)\s*$")
...
def parse_asm_report(asm_text: str):
    ...
    return raw_cycles, scaled_cycles, warm_loop_cycles, last_wmma
...
def run_trial(params: Dict[str, Any], output_s: str = OUTPUT_S):
    llc_cmd = [
        ...
        f"-amdgpu-resource-balancing-epi={params['resource_balancing']}",
        f"-amdgpu-ds-latency-epi={params['ds_latency']}",
        f"-amdgpu-ds-latency-split-epi={params['ds_latency_split']}",
        f"-amdgpu-ds-fifo-latency-epi={params['ds_fifo_latency']}",
        f"-amdgpu-signal-latency-epi={params['signal_latency']}",
        f"-amdgpu-ds-fence-latency-epi={params['ds_fence_latency']}",
        f"-amdgpu-ds-fifo-size-epi={params['ds_fifo_size']}",
        f"-amdgpu-ignore-valu-resource-balancing-epi={'true' if params['ignore_valu'] else 'false'}",
        f"-amdgpu-avoid-exp-final-islot-epi={'true' if params['avoid_exp_final_islot'] else 'false'}",
        f"-amdgpu-shadow-mix-epi={'true' if params['shadow_mix'] else 'false'}",
        f"-amdgpu-shadow-mix-wmma-min-valu1c-epi={params['shadow_mix_wmma_min_valu1c']}",
        f"-amdgpu-shadow-mix-wmma-min-ds-epi={params['shadow_mix_wmma_min_ds']}",
        f"-amdgpu-shadow-mix-wmma-min-salu-epi={params['shadow_mix_wmma_min_salu']}",
        f"-amdgpu-shadow-mix-lookahead-depth-epi={params['shadow_mix_lookahead_depth']}",
        f"-amdgpu-shadow-mix-max-blocking-cost-epi={params['shadow_mix_max_blocking_cost']}",
        f"-amdgpu-shadow-mix-max-candidates-epi={params['shadow_mix_max_candidates']}",
        f"-amdgpu-shadow-priority-wmma-over-ds-epi={'true' if params['shadow_priority_wmma_over_ds'] else 'false'}",
        f"-amdgpu-shadow-priority-wmma-over-salu-epi={'true' if params['shadow_priority_wmma_over_salu'] else 'false'}",
        f"-amdgpu-shadow-priority-cvt-over-ds-epi={'true' if params['shadow_priority_cvt_over_ds'] else 'false'}",
        f"-amdgpu-shadow-priority-cvt-over-salu-epi={'true' if params['shadow_priority_cvt_over_salu'] else 'false'}",
        f"-amdgpu-shadow-priority-trans32-over-valu1c-epi={'true' if params['shadow_priority_trans32_over_valu1c'] else 'false'}",
        f"-amdgpu-shadow-defer-trans32-epi={'true' if params['shadow_defer_trans32'] else 'false'}",
        f"-amdgpu-shadow-mix-trans32-min-valu1c-epi={params['shadow_mix_trans32_min_valu1c']}",
        f"-amdgpu-shadow-prefer-valu-over-salu-for-trans-epi={'true' if params['shadow_prefer_valu_over_salu_for_trans'] else 'false'}",
        f"-amdgpu-resource-priority-coexec-producer-epi={'true' if params['resource_priority_coexec_producer'] else 'false'}",
    f"-amdgpu-resource-priority-coexec-windows-size-epi={'true' if params['resource_priority_coexec_windows_size'] else 'false'}",
    f"-amdgpu-resource-priority-coexec-exposed-cycles-epi={'true' if params['resource_priority_coexec_exposed_cycles'] else 'false'}",
        f"-amdgpu-use-shadow-mix-rules-epi={'true' if params['use_shadow_mix_rules'] else 'false'}",
    ]
    ...
    raw_cycles, scaled_cycles, warm_loop_cycles, last_wmmma = parse_asm_report(asm_text)
...
def build_llc_cmd(p: Dict[str, Any], output_file: str = "res.s", input_file: str = BASE_LL) -> str:
    ...
    return (
        ...
        f"-amdgpu-resource-balancing-epi={p['resource_balancing']} "
        f"-amdgpu-ds-latency-epi={p['ds_latency']} "
        f"-amdgpu-ds-latency-split-epi={p['ds_latency_split']} "
        f"-amdgpu-ds-fifo-latency-epi={p['ds_fifo_latency']} "
        f"-amdgpu-signal-latency-epi={p['signal_latency']} "
        f"-amdgpu-ds-fence-latency-epi={p['ds_fence_latency']} "
        f"-amdgpu-ds-fifo-size-epi={p['ds_fifo_size']} "
        f"-amdgpu-ignore-valu-resource-balancing-epi={'true' if p['ignore_valu'] else 'false'} "
        f"-amdgpu-avoid-exp-final-islot-epi={'true' if p['avoid_exp_final_islot'] else 'false'} "
        f"-amdgpu-shadow-mix-epi={'true' if p['shadow_mix'] else 'false'} "
        f"-amdgpu-shadow-mix-wmma-min-valu1c-epi={p['shadow_mix_wmma_min_valu1c']} "
        f"-amdgpu-shadow-mix-wmma-min-ds-epi={p['shadow_mix_wmma_min_ds']} "
        f"-amdgpu-shadow-mix-wmma-min-salu-epi={p['shadow_mix_wmma_min_salu']} "
        f"-amdgpu-shadow-mix-lookahead-depth-epi={p['shadow_mix_lookahead_depth']} "
        f"-amdgpu-shadow-mix-max-blocking-cost-epi={p['shadow_mix_max_blocking_cost']} "
        f"-amdgpu-shadow-mix-max-candidates-epi={p['shadow_mix_max_candidates']} "
        f"-amdgpu-shadow-priority-wmma-over-ds-epi={'true' if p['shadow_priority_wmma_over_ds'] else 'false'} "
        f"-amdgpu-shadow-priority-wmma-over-salu-epi={'true' if p['shadow_priority_wmma_over_salu'] else 'false'} "
        f"-amdgpu-shadow-priority-cvt-over-ds-epi={'true' if p['shadow_priority_cvt_over_ds'] else 'false'} "
        f"-amdgpu-shadow-priority-cvt-over-salu-epi={'true' if p['shadow_priority_cvt_over_salu'] else 'false'} "
        f"-amdgpu-shadow-priority-trans32-over-valu1c-epi={'true' if p['shadow_priority_trans32_over_valu1c'] else 'false'} "
        f"-amdgpu-shadow-defer-trans32-epi={'true' if p['shadow_defer_trans32'] else 'false'} "
        f"-amdgpu-shadow-mix-trans32-min-valu1c-epi={p['shadow_mix_trans32_min_valu1c']} "
        f"-amdgpu-shadow-prefer-valu-over-salu-for-trans-epi={'true' if p['shadow_prefer_valu_over_salu_for_trans'] else 'false'}"
        f"-amdgpu-resource-priority-coexec-producer-epi={'true' if p['resource_priority_coexec_producer'] else 'false'} "
        f"-amdgpu-resource-priority-coexec-windows-size-epi={'true' if p['resource_priority_coexec_windows_size'] else 'false'} "
        f"-amdgpu-resource-priority-coexec-exposed-cycles-epi={'true' if p['resource_priority_coexec_exposed_cycles'] else 'false'} "
        f"-amdgpu-use-shadow-mix-rules-epi={'true' if p['use_shadow_mix_rules'] else 'false'}"
    )
